...
$column_width['reference']	=	"120px";
$column_width['memo']		=	"250px";
...
if(!empty($entries)) {
		...
		<table border="0" cellpadding="1" style="font-size:10px;">
			<thead>
				<tr>
					...
					<th style="font-size:12px; text-align:center; background-color:#000; color:#FFF;">Balance</th>
				</tr>
			</thead>
		</table>
		';
	...
	foreach($entries as $docctr=>$entry) {
		...
		if(!empty($entry)) {
			if($entry['stat']==1) {
				...
				$tablecontents	.=	'<td style="text-align:right;">'.($balance!=0?number_format($balance,2):'-').'</td>';
				$tablecontents	.=	'</tr>';
	...
	$footer	.=	'<th style="font-size:10px; text-align:right;">'.number_format($balance,2).'</th>';
	$footer	.=	'</tr>';
	$footer	.=	'<tr>';
	$footer	.=	'<th style="width:'.$column_width['date'].';"></th>';
	$footer	.=	'<th style="width:'.$column_width['reference'].';"></th>';
	...
	$footer	.=	'<th></th>';
	$footer	.=	'<th></th>';
	$footer	.=	'<th colspan="3" style="text-align:right;">===================================</th>';
	$footer	.=	'</tr>';
