...
<div class="panel panel-primary">
	...
	<div class='panel-body'>
		...
		<form id='inventory_issuance_form' role='form' action='<?php echo HTTP_PATH."accounting/inventory_issuances/insert/for_clearing/".$MSIS[0]['id']?>' method='POST' enctype='multipart/form-data'>

		<input type='hidden' id='company_id' name='company_id' value='<?php echo $MSIS[0]['company_id'];?>'>

		<div class='tile-stats col-xs-12 col-md-12'>
			<br><label>Materials and Supplies Issuance Details</label><br>

			<table class="table">
				<tr>
					<td class='lbl'>MSIS #:</td>
					<td><?php echo $MSIS[0]['control_number'] . " (Transaction Code No.".$MSIS[0]['id'].")"?></td>
					<td class='lbl'>Date Issued:<td>
					<td><?php echo date("j F Y h:i A", strtotime($MSIS[0]['issue_date']));?></td>
				</tr>
				<tr>
					<td class='lbl'>Company:</td>
					<td><?php echo $MSIS[0]['company'];?></td>
					<td class='lbl'>Location:<td>
					<td><?php echo $MSIS[0]['from_location'];?></td>
				</tr>
				<tr>
					<td class='lbl'>Issued To:</td>
					<td><?php echo $MSIS[0]['issued_to'];?></td>
					<td class='lbl'>Issued For:<td>
					<td><?php echo $MSIS[0]['issued_for'];?></td>
				</tr>
				<tr>
			...
			<table class="table table-bordered table-striped">
				...
				<tr>
					...
					<input type='hidden' id='msis_amount' value='<?php echo number_format($total_amount,2,'.','');?>'readonly>
				</tr>
				</tbody>
			</table>
		</div>
									...
									$total_credit = 0;
										...
										if(empty($transaction)) {
											echo "<tr id='row_entry0' class='tbl_row_entry'>
													...
													<td style='text-align:right'><input type='hidden' id='credit[]' name='credit[]' class='form-control credit_amount'  value='".$total_amount."' style='text-align:right' readonly>".number_format($total_amount,2,'.',',')."
													</td>
													<td align='center'>
					...
					<?php if(!empty($transaction)) {?>
						<?php if($transaction['stat']==0){?>
							<?php if($this->Abas->checkPermissions("accounting|approve_vouchers",false)){ ?>
								<input type="button" class="btn btn-success btn-m" onclick="javascript:approveInventoryIssuance(<?php echo $MSIS[0]['id'];?>,<?php echo $transaction['id'];?>);" value="Approve"/>
							<?php } ?>
						<?php } ?>
...
function checkForm() {
    ...
    /*if( document.getElementById("attach_file[0]").files.length == 0 ){
    	msg+="Please make sure that an Approved and Signed Copy of MSIS is attached! <br/>";
	}*/

	var flag_debit_credit = 0;
	var flag_credit_msis = 0;
    $("#table_entries input").each(function() {
	      ...
	      var total_credit_amount = 0;
  		  ...
  		  var msis_amount = parseFloat($('#msis_amount').val()).toFixed(2);
		  ...
		  //console.log(msis_amount + "--" + total_credit_amount);

		  if(total_debit_amount != total_credit_amount){
		  	  flag_debit_credit = 1;
		  }else{
		  	  flag_debit_credit = 0;
		  	  if(total_credit_amount != msis_amount){
		  	  	flag_credit_msis = 1;
		  	  }else{
		  	  	flag_credit_msis = 0;
		  	  }
    ...
    if(flag_credit_msis==1){
    	msg+="Total Credit amount is not equal with total MSIS amount! <br/>";
    }
	...
	if(msg!="") {
		...
		bootbox.confirm({
		    ...
		    callback: function (result) {
		    	if(result){
			        document.getElementById("inventory_issuance_form").submit();
			        return true;
		    	}
		    }
		});
...
function approveInventoryIssuance(msis_id,transaction_id) {
		...
		bootbox.confirm({
		    ...
		    callback: function (result) {
		    	if(result==true){
					  window.location.href = "<?php echo HTTP_PATH;?>accounting/inventory_issuances/approve/for_posting/" + msis_id + "/" + transaction_id;
				}
		    }
		});
