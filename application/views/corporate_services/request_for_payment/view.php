...
<?php if($request[0]['status']=='For Voucher' || $request[0]['status']=='For voucher' || $request[0]['status']=='For releasing'){?>
	<a href="<?php echo HTTP_PATH.'Corporate_Services/request_for_payment/print/'.$request[0]['id']?>" class="btn btn-info exclude-pageload" target="_blank">Print</a>
<?php } ?>

<?php if($request[0]['status']=='For Verification'){?>
	<a class="btn btn-warning btn-m exclude-pageload" data-toggle='modal' data-target='#modalDialog' href="<?php echo HTTP_PATH.'Corporate_Services/request_for_payment/edit/'.$request[0]['id']?>">Edit</a>
...
<a href="<?php echo HTTP_PATH.'Corporate_Services/request_for_payment/listview'?>" class="btn btn-dark exclude-pageload">Back</a>
</div>
...
<div class="panel panel-primary">
	...
	<div class="panel-body" style="overflow: auto">
		...
		<h4 class="text-center"><?php echo $company->address; ?></h4>
	...
	bootbox.prompt({
				    ...
				    callback: function (result) {
				    	if(result==null || result==""){
				    		...
				    		$.ajax({
							     type:"POST",
							     url:"<?php echo HTTP_PATH;?>Corporate_Services/request_for_payment/cancel/"+rfp_id,
							     data: {comments:result}
							  });
				    		window.location.href = "<?php echo HTTP_PATH;?>Corporate_Services/request_for_payment/view/" + rfp_id;
				    	}
