<h2 id="glyphicons-glyphs">Accountability Forms</h2>
<?php if($this->Abas->checkPermissions("asset_management|add_accountability_form",FALSE)){?>
	<a href="<?php echo HTTP_PATH.'Asset_Management/accountability_form/add';?>" class="btn btn-success exclude-pageload" data-toggle="modal" data-target="#modalDialog" data-backdrop="static" data-keyboard="false">Add</a>
<?php }?>
<a href="<?php echo HTTP_PATH."Asset_Management/accountability_form/listview/" ?>" class="btn btn-dark force-pageload">Refresh</a>
<table data-toggle="table" id="data-table" class="table table-bordered table-striped table-hover" data-url="<?php echo HTTP_PATH.'Asset_Management/accountability_form/load'?>" data-cache="false" data-side-pagination="server" data-pagination="true" data-show-columns="true" data-sort-name="created_on" data-sort-order="desc" data-page-list="[5, 10, 20, 50, 100, 200, 500, 1000, 2500, 5000, 10000]" data-search="true" data-filter-control="true" data-filter-strict-search="false" data-export-data-type="all" data-show-export="true" data-export-types="['excel']">
	<thead>
		<tr>
...
<script>
	function details(value, row, index) {
		return [
			'<a class="btn btn-info btn-xs btn-block force-pageload" href="<?php echo HTTP_PATH.'Asset_Management/accountability_form/view'; ?>/'+ row['id'] +'">View</a>'
		].join('');
	}
	$(function () {
