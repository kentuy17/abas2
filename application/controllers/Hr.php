		...
		public function employee_profile($action="", $id="") {$data=array();
			...
			if($id=="") {
				...
				if(is_numeric($id)) {
					...
					$data['credit_code'] = $this->set_leave_credit($id);
...
//-----------------------------------------------------------------------------------------//
		...
		public function set_leave_credit($id,$parm1='',$parm2=''){
			if($id == 'set'){
				$where = ['id' => $parm1];
				$this->Abas->updateItem('hr_employees',['leave_credit_id'=>$parm2],$where);
				$getCredits = $this->leave_credits($parm1);
				if($getCredits){
					$this->Abas->updateItem('hr_employees',['leave_credits'=>$getCredits['credit']],$where);
				}
				redirect(HTTP_PATH.'hr/employee_profile/view/'.$parm1);
			}else{
				try {
					return $this->leave_credits($id);
				} catch (Exception $e) {
					//echo 'Caught exception: ',  $e->getMessage(), "\n";
					$obj = [
						'reg_date' => 0,
		...
		public function leave_credits($id){
			$emp = $this->Abas->getItemById('hr_employees',['id'=>$id]);
			$lc = $this->Abas->getItemById('leave_credits',['id'=>$emp->leave_credit_id]);

			if(!$lc){
				throw new Exception("Error Processing Request", 1);
			}

			$hired = strtotime($emp->date_hired);
			$reg_date = date("Y-m-d", strtotime("+6 month", $hired));
			$reg_year = date("Y",strtotime($reg_date));
			$perm_year = (date("Y") - $reg_year) + 1;
			$closing = $reg_year."-12-31";
			$opening = $reg_year."-01-31";
			$days_in_year = $this->dateDifference($opening,$closing);

			$max_leave = $lc->max_vl + $lc->max_sl;
			$inc = $lc->vl_inc + $lc->sl_inc;
			if($perm_year == 1){
				$pro_rated_days = $this->dateDifference($reg_date, $closing);
				$pro_rated = ($pro_rated_days / $days_in_year) * $lc->base_credit;
				$credits = number_format($pro_rated,2);
			}elseif ($perm_year > 1) {
				$total = $lc->base_credit + ($inc * ($perm_year - 1));
				if($total >= $max_leave){
					$credits = $max_leave;
				}else{
					$credits = $total;
